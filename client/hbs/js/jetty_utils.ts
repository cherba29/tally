...
interface JettyResponseData {
  ...
  summaries: {[ownerAccountType: string]: {
...
function jettyToBalance(jettyBalance: JettyBalance|null|undefined): Balance|undefined {
  if (!jettyBalance || jettyBalance.amount === null) { return undefined; }
  return new Balance(jettyBalance.amount, new Date(jettyBalance.date), jettyBalance.type as BalanceType);
}
...
export function transformJettyBudgetData(data: JettyResponseData): MatrixDataView {
  ...
  for (const [ownerAccountType, monthToJettySummaryStatement] of Object.entries(data.summaries)) {
    const monthSummaryStatements: { [month: string]: SummaryStatement; } = summaries[ownerAccountType] = {};
  ...
  return transformBudgetData(
      data.months, accountNameToAccount,
