...
const HEADER_ROW = [
  'Account', 'Type','OpenedOn','ClosedOn','External','Closed',
  'Year','Month','Start Amount','Start Projected', 'End Amount','End Projected',
  'Inflows','OutFlows','Income','Expense','Transfers','Unaccounted'
];
...
export const report: CommandModule<unknown, ReportOptions> = {
  ...
  handler:  ({ account, startMonth, endMonth }): void => {
    ...
    for (const transactionStatement of statementTable) {
      ...
      if (stmtAccount.name !== account) { continue; }
      const row: string[] = [
        stmtAccount.name,
        stmtAccount.type,
        ...
        transactionStatement.startBalance ? (transactionStatement.startBalance?.amount/100).toFixed(2) : '',
        transactionStatement.startBalance ? (transactionStatement.startBalance.type === BalanceType.PROJECTED ? 'P' : 'C') : '',
        transactionStatement.endBalance ? (transactionStatement.endBalance.amount/100).toFixed(2) : '',
        transactionStatement.endBalance ? (transactionStatement.endBalance.type === BalanceType.PROJECTED ? 'P' : 'C') : '',
        ...
        transactionStatement?.unaccounted ? (transactionStatement.unaccounted/100).toFixed(2) : ''
      ];
