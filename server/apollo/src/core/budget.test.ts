...
test('build simple', () => {
  ...
  const account1 = new Account({
    name: 'test-account1', type: AccountType.EXTERNAL, owners: []
  });
  const account2 = new Account({
    name: 'test-account2', type: AccountType.BILL, owners: []
  });
  const account3 = new Account({
    name: 'test-account3', type: AccountType.BILL, owners: []
  });
  ...
  expect(budget.months).toEqual([
    'Nov2019', 'Dec2019', 'Jan2020', 'Feb2020'
  ].map(Month.fromString));
